 ...
 * 新飘花电影网
 */
public class CLPH extends Spider {
    ...
    private String response(Response response) throws Exception {
    ...
    public String homeContent(boolean filter) throws Exception {
        ...
        List<String> typeIds = Arrays.asList("/dongzuo/", "/xiju/", "/aiqing/", "/kehuan/", "/juqing/", "/xuanyi/", "/zhanzheng/", "/kongbu/", "/zainan/", "/jilu/", "/lianxuju/", "/dongman/", "/zongyi/");
        List<String> typeNames = Arrays.asList("动作片", "喜剧片", "爱情片", "科幻片", "剧情片", "悬疑片", "战争片", "恐怖片", "灾难片", "纪录片", "连续剧","动漫", "综艺");
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) throws Exception {
        ...
        String html = response(OkHttp.newCall(cateUrl, getHeader()));
        ...
        for (Element item : items) {
            ...
            String remark = item.select("span").text().replace("点击下载", "");
    ...
    public String detailContent(List<String> ids) throws Exception {
        ...
        String html = response(OkHttp.newCall(detailURL, getHeader()));
    ...
    public String searchContent(String key, boolean quick, String pg) throws Exception {
        String searchURL = siteUrl + "/plus/search.php?q=" + URLEncoder.encode(key, "GBK") + "&searchtype.x=0&searchtype.y=0&PageNo=" + pg;
        String html = response(OkHttp.newCall(searchURL, getHeader()));
        JSONArray videos = new JSONArray();
        Elements items = Jsoup.parse(html).select("#list dl");
        for (Element item : items) {
            String vid = item.select("strong a").attr("href");
